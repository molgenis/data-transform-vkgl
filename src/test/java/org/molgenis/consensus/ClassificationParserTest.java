...
class ClassificationParserTest {
  ...
  void parse() {
    ...
    Classification classification = Classification.builder()
        .geneVariant(GeneVariant.builder().chromosome("1").start(12059085).stop(12059086)
            .ref("G").alt("GA")
            .gene("MFN2").build()).pathogenicity(Pathogenicity.VUS).build();
    assertEquals(singletonList(classification), classificationParser.parse(new StringReader(
        tsv)));
  }
}