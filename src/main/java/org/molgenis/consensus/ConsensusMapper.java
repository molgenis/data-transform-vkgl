...
import org.molgenis.consensus.model.Consensus.ConsensusBuilder;
import org.molgenis.consensus.model.ConsensusType;
import org.molgenis.consensus.model.GeneVariant;
import org.molgenis.consensus.model.Lab;
import org.molgenis.consensus.model.Pathogenicity;

public class ConsensusMapper {

  public Consensus map(GeneVariant geneVariant, Map<Lab, Classification> labClassifications) {
    ConsensusBuilder builder = Consensus.builder().chromosome(geneVariant.getChromosome())
        .start(geneVariant.getStart()).stop(geneVariant.getStop()).ref(
            geneVariant.getRef()).alt(geneVariant.getAlt()).gene(geneVariant.getGene())
  ...
  private static void setLabPathogenicity(Map<Lab, Classification> labClassifications,
      ConsensusBuilder builder) {
    labClassifications.forEach((lab, classification) -> {
  ...
  private static void setConsensus(Map<Lab, Classification> labClassifications,
      ConsensusBuilder builder) {
    Set<Pathogenicity> pathogenicitySet = labClassifications.values().stream()
        .map(Classification::getPathogenicity)
        .collect(toSet());
    ...
    if (pathogenicitySet.size() == 1) {
      ...
      consensusType = AGREEMENT;
    } else if (pathogenicitySet.contains(VUS) && (containsBenignFlavor(pathogenicitySet)
        && !containsPathogenicFlavor(pathogenicitySet)) || (!containsBenignFlavor(pathogenicitySet)
